...
# start GPU Miner
CMD ./aionminer -cd $DEVICE -cv $CUDA_SOLVER -cb $NUMBER_BLOCKS -ct $NUMBER_THREADS -l $MINING_POOL_ADDRESS:$MINING_POOL_PORT


#   docker build -f aion-gpu-miner.Dockerfile -t aion:gpu-miner .
#   docker run -it -e MINING_POOL_ADDRESS=solo-mining-pool --net=miningaioncoins_default --runtime=nvidia --rm --name gpu_miner aion:gpu-miner bash
